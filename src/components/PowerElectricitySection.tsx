import type { PowerElectricitySourceEvidence } from "@/data/powerElectricity";
...
function VerificationEvidence({
  ...
  evidence: PowerElectricitySourceEvidence[];
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <div className="max-w-3xl">
        ...
        <h3 className="mt-2 text-xl font-bold text-slate-950">
          Public power-sector source references
        </h3>
        <p className="mt-3 text-sm leading-6 text-slate-700">
          Power and electricity indicators are linked directly to public PIB
          pages. The locally stored PDF is provided as a secondary evidence copy
          for the public power-sector release.
        </p>
      </div>
...
export function PowerElectricitySection() {
  const shortageRows = powerElectricity.powerShortage.data.map(
    (item, index) => ({
      label: item.year,
      value: item.value,
  ...
  const discomProfitRows = powerElectricity.discoms.profitLoss.map(
    (item, index) => ({
      label: item.year,
    ...
    <section id="power-electricity" className="bg-slate-50">
      <div className="mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          <BarChart
            title={powerElectricity.powerShortage.title}
            rows={shortageRows}
          />
          <BarChart
            title={powerElectricity.dailySupply.title}
            rows={supplyRows}
          />
          <BarChart
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          ...
          <MetricCard
            ...
            value={
              powerElectricity.renewableEnergy.pmSuryaGhar.displayHouseholds
            }
        ...
        <div className="mt-8">
          <VerificationEvidence
            evidence={powerElectricity.verificationEvidence}
          />
        </div>
